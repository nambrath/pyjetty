...
class ProcessMCBase(process_base.ProcessBase):
	...
	def initialize_config(self):
		...
		self.do_emb_rotation = config['do_emb_rotation']
		self.do_feedin_check = config['do_feedin_check']
	...
	def process_mc(self):
		...
		if self.do_median_subtraction:
			...
			for jetR in self.jetR_list:
				...
				self.median_subtractor_truth[jetR] = fj.JetMedianBackgroundEstimator(self.jet_selector_medsub[jetR], self.jet_def_medsub[jetR], fj.AreaDefinition(fj.active_area_explicit_ghosts))
				...
				for i, R_max in enumerate(self.max_distance):
					...
					if self.do_median_subtraction:
						...
						self.analyze_jets(jets_combined_reselected, jets_truth_selected, jets_truth_selected_matched, jetR,
														...
														fj_particles_truth_holes = fj_particles_truth_holes, rho_bge = (rho,rho_truth), fj_particles_det_cones = fj_particles_combined_beforeCS, fj_particles_truth_cones = fj_particles_truth)
						
						self.analyze_perpendicular_cone(jets_combined_reselected, jets_det_pp_selected, jets_truth_selected, jets_truth_selected_matched, jetR, fj_particles_combined_beforeCS, rho)
	...
	def analyze_jets(self, jets_det_selected, jets_truth_selected, jets_truth_selected_matched, jetR,
		...
		for jet_det in jets_det_selected:
			...
			self.fill_det_before_matching(jet_det, jetR, R_max, rho_bg)
		...
		for jet_truth in jets_truth_selected:
			...
			if self.is_pp or self.fill_Rmax_indep_hists:
				self.fill_truth_before_matching(jet_truth, jetR, rho_bg_truth)

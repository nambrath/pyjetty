...
def download_data(config_file):
    ...
    output_dir = os.path.join(output_dir)#, period_out)
...
def download(train_output_dir, run, pt_hat_bin=None):
    ...
    with open(temp_filelist_name) as f:
        ...
        subdirs = [ x.strip(" /") for x in subdirs_all if x.strip(" /").isdigit() ]
    ...
    for subdir in subdirs:
        ...
        with open('log_{}.txt'.format(run), "a") as logfile:
            cmd = 'alien_cp alien://{}/{}/AnalysisResults.root file:{}'.format(train_output_dir, subdir, subdir_path)
